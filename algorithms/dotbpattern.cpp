...
tuple<string, float> sep_defect(string line){
    string s=line;
    float d=nan("");
    size_t p = 0;
    while((p = line.find('\t')) != string::npos){
        ...
        d = strtof(line.substr(p+1).c_str(), NULL);
        line.erase(0, p + 1);
        if(d == 0.0F)
            d = nan("");
    }
...
tuple<vector<Node *>, vector<float>, vector<string>> load_motifs(ifstream *input_file){
    string line;
    string s;
    float d;
    vector<Node *> trees;
    ...
    while(getline(*input_file, line)){
        std::tie(s, d) = sep_defect(line);
        Node* tree = tree_from_string(s, true)[0];
    ...
    return make_tuple(trees, defects, dbns);
}

vector<vector<Node *>> load_from_file(ifstream *input_file, int line_offset = 0) {
    /*
...
int main(int argc, char *argv[]) {
    ...
    vector<float> defects;
    vector<string> pattern_dbn;
    std::tie(patterns, defects, pattern_dbn) = load_motifs(&pattern_file);
    pattern_file.close();
    ...
    for (int f = 0; f < input_files.size(); f++) {
        ...
        for (int m = 0; m < trees.size(); m++) {  // For each model
            ...
            for (int c = 0; c < chains.size(); c++) {  // And for each model
                ...
                output_file << name << "-" << m+1 << "-" << c+1 << '\t';

                for (int p = 0; p < patterns.size(); p++) {
                    vector<array<int, 2>> positions = find_pattern(chain, patterns[p]);  
                    if (positions.size() > 0) {
                        // ex : 99:12-19;58-75;
                        output_file << pattern_dbn[p];
                        if(!isnan(defects[p]))
                            output_file << defects[p];
                        output_file << ":";
